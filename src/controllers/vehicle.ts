...
import {
  VehicleAvailability,
  VehicleService,
} from "../providers/services/vehicle";
...
export class VehicleController {
  ...
  async free(@Query() query: Free): Promise<VehicleAvailability[]> {
    ...
    // Push each item from the VehicleAvailability map into an array
    let vehicleAvailabilities: VehicleAvailability[] = [];
    (
    ...
    ).forEach((vehicleAvailability, _) =>
      vehicleAvailabilities.push(vehicleAvailability)
    );
